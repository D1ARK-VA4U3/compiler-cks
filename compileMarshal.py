...
def main():
    ...
    try:
        ...
        if chos == '1' or chos == '01':
            ...
            file = raw_input('%s[%s\xe2\x9b\xa5%s] %sEnter Your File Location >> %s' % (b, i, b, gr, i))
            cot = int(raw_input('%s[%s\xe2\x9b\xa5%s] %sCount >> %s' % (b, m, b, gr, i)))
            if cot < 400:
                out = file.replace('.py', '') + '_enc.py'
                oa = open(file).read()
                ni = compile(oa, '<D1ARK-VA4U3>', 'exec')
                bo = marshal.dumps(ni)
                ab = repr(bo)
                s = open(out, 'w')
                s.write('#Compile By D1ARK-VA4U3\n#My Team : Cyber King Squad\nimport marshal\nexec(marshal.loads(' + str(ab) + '))')
                s.close()
                while True:
                    if cot >= cout:
                        nz = open(out).read()
                        dn = compile(nz, '<D1ARK-VA4U3>', 'exec')
                        bx = marshal.dumps(dn)
                        nl = repr(bx)
                        ns = open(out, 'w')
                        ns.write('#Compile By D1ARK-VA4U3\n#My Team : Cyber King Squad\nimport marshal\nexec(marshal.loads(' + str(nl) + '))')
                        ns.close()
                        cout += 1
                        continue
                    break
            ...
            if cot < 400:
                ...
                xn = open(out, 'w')
                xn.write("#Compile By D1ARK-VA4U3\n#My Team : Cyber King Squad\nimport base64\nexec(base64.b32decode('%s'))\n" % nl)
                xn.close()
                print '\x1b[34m[\x1b[31m!\x1b[34m] \x1b[37mDone Di Simpan \x1b[32m[ \x1b[37m%s \x1b[32m] \x1b[37m!' % out
                raw_input('%s[%s\xe2\x9d\x97%s] %sBack %s\xe2\x9e\xa4 %s' % (b, m, b, gr, i, cg))
                main()
            ...
            b = []
            bin = ''
            xUn = compile(bc, '<D1ARK-VA4U3>', 'exec')
            ...
            if cot < 200:
                out = file.replace('.py', '') + '_enc.py'
                oa = open(file).read()
                cpa = compile(oa, '<D1ARK-VA4U3>', 'exec')
            ...
            b = []
            bin = ''
            xh = compile(bc, '<D1ARK-VA4U3>', 'exec')
            pg = marshal.dumps(xh)
            ...
            bva = open(js).read()
            bgx = open(js, 'w')
            _bLos = compile(bva, '<D1ARK-VA4U3>', 'exec')
            _vjd = marshal.dumps(_bLos)
            gz = repr(_vjd)
            bgx.write('#Compile By D1ARK-VA4U3\n#My Team : Cyber King Squad\nimport marshal\nexec(marshal.loads(' + str(gz) + '))')
            bgx.close()
            py_compile.compile(js)
            gb = open(js + 'c').read()
            bi = open(js, 'w')
            bi.write(gb + '\n\n\n\tOBFUSCATE BY D1ARK-VA4U3 | MY TEAM : Cyber King Squad\n\tNOTE : JANGAN DI EDIT NANTI ERROR !\n\n')
            bi.close()
            os.system('rm ' + js + 'c')
            print '\x1b[34m[\x1b[31m!\x1b[34m] \x1b[37mDone Di Simpan \x1b[32m[ \x1b[37m%s \x1b[32m] \x1b[37m!' % js
            raw_input('%s[%s\xe2\x9d\x97%s] %sBack %s\xe2\x9e\xa4 %s' % (b, m, b, gr, i, cg))
            ...
            f = open(js, 'w')
            f.write('import binascii,base64\ng = ' + str(b) + "\ns = '" + str(e) + "'" + "\neval(compile(base64.b32decode(''.join([chr(i) for i in g])+binascii.unhexlify('%x' % int(s, 2))),'<D1ARK-VA4U3>','exec'))")
            f.close()
            py_compile.compile(js)
            gb = open(js + 'c').read()
            bi = open(js, 'w')
